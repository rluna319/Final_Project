...
# The following code is from https://github.com/boppreh/aes/blob/master/aes.py
...
if __name__ == '__main__':
    ...
    # Test to encrypt a single block
    ...
    # print(pt)
    #
    # ct = test.encrypt_block(pt)
    #
    # print(ct)
    #
    # dct = test.decrypt_block(ct)
    #
    # print(dct)

    test = AES(k)

    # pt = b' "The secret of getting ahead is getting started." - Mark Twain'
    #
    # print("\nOriginal PlainText: "+str(pt)+"\n")
    #
    # # this initialization vector (iv) is for testing purposes only
    iv = b'\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\xff'
    #
    # ct = test.encrypt_cbc(pt, iv)
    #
    # print("Cipher Text: "+str(ct)+"\n")
    #
    # dct = test.decrypt_cbc(ct, iv)
    #
    # print("Decrypted Output: "+str(dct)+"\n")

    img_path = "../test/_qhamM5iRoQ.png"

    img_array = openImage(img_path)

    out_array = img_array.flatten()

    print(out_array)

    #pt = out_array.tobytes()
    pt = b'This is a test. '

    pt = pt * 600000

    print("\nLength of image byte array: " + str(len(pt)))
